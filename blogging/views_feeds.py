...
def feed( request, author = None, selected_tags = None ):
   ...
   if ( selected_tags is not None ):
      articles = articles.filter( tags__url_name__in = selected_tags.split( "," ) )
      
   feed = feedgenerator.Atom1Feed( "thefoundation", 
                                   settings.HOST_NAME, 
   ...
   for article in articles:
      feed.add_item( title = article.title, 
                     ...
                     description = "<p>%s</p>%s" % ( article.html_teaser, article.html ),
                     subtitle = article.html_teaser,
                     author_name = "%s %s" % ( article.author.first_name, article.author.last_name ),
                     author_link = "%s/about/%s" % ( settings.HOST_NAME, article.author.username ),
