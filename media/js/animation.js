// Animate properties to mutable target values, using a model of acceleration and deceleration.
var Animation = ( function()
{
    ...
    * @param Object optionalOrigin The object that triggered the change, it won't be notified.
    */
   ...
   /** Encapsulate a css property of a html element and allow to access it as a value between zero and one. 
    *
    * The value of zero is mapped to the lower limit, the value of one to the upper limit. Observers can
    * register, to be notified when value changes. 
    * 
    * Implement your own range proxy to control other things such as percentages, colors or even non-css
    * values such es html text-data. Simply extend and override the methods {@link #updateView} and 
    * {@link #updateValue}.
    *
    * @param QuerySet querySet A jQuery set to control.
    * @param String propertyName The css property to control.
    * @param ValueHolder limits A source of min/max value, to which zero and one are mapped.
    */
   ...
   {
      this.v_ = ( parseFloat( this.qs_.css( this.p_ ) ) - this.min_ ) / this.delta_;
   }

   /** Update the DOM state to the current value, this is used during animation.
    * @protected
    */
   CssRangeProxy.prototype.updateView = function( value )
   {
      // console.log( "updateView: p=%o, min=%o, value=%o, delta=%o, unit=%o", 
      //              this.p_, this.min_, value, this.delta_, this.unit_ );
   ...
   // To avoid conflicts, the value is simply updated using the first channel that has non-zero delta.
   ColorRangeProxy.prototype.updateValue = function()
   {
      ...
      this.v_ = ( colors.parse( this.qs_.css( this.p_ ) )[ i ] - this.min_[ i ] ) / this.delta_[ i ];
   }
   ...
   /** Proxy to animate individual channels of css color properties independently.
    *
   ...
   {
      // Update value using selected channel.
      this.v_ = ( colors.parse( this.qs_.css( this.p_ ) )[ this.channel_ ] - this.min_ ) / this.delta_;
   }
   ...
   /**
    * Create an animation to control one css-property of an html element. The animation is always performed
    * from current value to the current target value. Thus the only way to control the animation itself is to
    * control the target value.
    *
    * Both of these values may be changed at any time from the outside.
    *
    * @param CssRangeProxy proxy A renge proxy to be controlled by the animation. The proxy also provides the
    *                            initial value for the animation.
    * @param Function optionalCallback A method to call when the animation has finished.
    *
    * @return ValueHolder Contains the current value between 0 and 1. Call set( ... ) to start the 
    *                     animation to the new target value.
    */
    ...
    */
   Animation.forPixelRange = function( querySet, propertyName, optionalFrom, optionalTo )
   {
      return Animation.create( new CssRangeProxy( querySet,
                                                  propertyName,
                                                  Animation.limits( optionalFrom || 0, optionalTo || 0, "px" ) 
      ) );
   }
   ...
   /**
    * @return ValueHolder A new observable proxy with the given values for the "min" and "max" properties.
    */
   ...
   /** Create three animations, one for each channel of the given color property.
    *
    * The resulting three targets are returned in an array.
    */
   Animation.forChannels= function( querySet, propertyName )
   {
      return [ Animation.create( Animation.channelProxy( querySet, propertyName, R ), Length.SHORT ),
               Animation.create( Animation.channelProxy( querySet, propertyName, G ), Length.SHORT ),
               Animation.create( Animation.channelProxy( querySet, propertyName, B ), Length.SHORT ) ];      
   }
    ...
    * @param String propertyName The property to animate on the given set. A rangeProxy is created, to
    *                            convert between css values and the [0...1] range used by the animation.
    * @param ValueHolder optionalLimits Updateable min/max values for range conversion.
    */
   Animation.cssProxy = function( querySet, propertyName, optionalLimits )
   {
      return new CssRangeProxy( querySet, 
                                propertyName, 
   ...
   /**
    * Animates a css color value so that all channels reach the target at the same time.
    */
   Animation.colorProxy = function( querySet, propertyName, optionalLimits )
   {
      return new ColorRangeProxy( querySet, 
                                  propertyName, 
    ...
    */
   Animation.channelProxy = function( querySet, propertyName, channel, optionalLimits )
   {
      return new ChannelRangeProxy( querySet, 
                                    propertyName, 
                                    channel,
   ...
   /**
    * @return Boolean <tt>false</tt> if this animation has more steps, <tt>true</tt> if it is done.
    */
   function step( self )
   {
      if ( self.done_ ) {
      ...
      // Calculate the acceleration, which depends on the velocity and on the remaining distance to 
      // the target. 
      // - If the velocity is not in the target direction, use maximum deceleration.
      // - Else, if the velocity has to be reduced to reach zero before the target is hit, use the lowest 
      //   possible (absolute) acceleration in range [ BRAKE_ACCELERATION ... BRAKE_ACCELERATION_MAX ].
      // - Else, if the max velocity has reached VELOCITY_MAX, zero acceleration is used.
      // - Else, the velocity is increased, using ACCELERATION.

      var a, v = self.velocity_, d = self.distance_, timeslice = self.timeslice_;

      // The signum of velocity and distance is important for the direction of the acceleration.
      var sgnV = v < 0 ? -1 : 1;
      var sgnD = d < 0 ? -1 : 1;

      // Stop/Brake-Acceleration; use it to reach a velocity of zero at the target.
      // NEEDS FIX B: The last factor should be 1, accordings to mechanics. But 0.7 works better - why?
      var stopAcceleration = ( 0.5 * ( -abs(v) / d ) + ( -(v*v) / d ) ) * 0.7
      var sgnSA = stopAcceleration < 0 ? -1 : 1;
      // The absolute value of the stop acceleration - for comparisions to the brake acceleration.
      ...
      // 
      else if ( abs( d ) < 0.5 && absoluteStopAcceleration > BRAKE_ACCELERATION ) {
         op = "--";
   ...
   // ...velocity is "units per animation-length".
   // ...acceleration is "units per animation-length" / animation-length.
   var DELAY = 20;
   ...
   /**
    *
    * @param Number d The remaining distance to the target, usually (but not necessarily) from [-1 ... 1].
    * @param Number v The current velocity, from [ -VELOCITY_MAX ... VELOCITY_MAX ].
    * @param Number timeslice The unit-fraction used to generate velocities and acceleration values.
    *
