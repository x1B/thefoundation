...
from blogging.models import Blog, Article, Tag


def render(template, request, dict):
    """General purpose helper."""
    context = RequestContext(request, dict = dict)
    return django.http.HttpResponse(loader.render_to_string(template, None, context))
...
def login_required_processor(request):
    ...
    raise django.http.Http404(handle_404(request))
...
def handle_403(request):
    ...
    return handle_error(request, django.http.HttpResponseForbidden(),
                        "403.html")
...
def handle_500(request):
    ...
    return handle_error(request, django.http.HttpResponseServerError(),
                        "500.html")
