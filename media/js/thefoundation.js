var thefoundation = ( function()
{
   ...
   var pageType = $( ".general_content" ).length ?
         GENERAL :
         ( $( "#people li.active" ).length ? BLOG : MAIN );

   var authors = [ "michael", "daniel", "david", "matthias" ];

   var columnHeightLimits =
      Animation.limits( labelMargin,
                        $( "#people" ).height() - labelMargin, "px" );
   ...
   if ( pageType != BLOG ) {
      ...
      for ( var i = 0; i < authors.length; ++i ) {
         var name = authors[ i ].charAt( 0 ).toUpperCase() +
                    authors[ i ].substring( 1 );
         $( "#stat-labels" ).append(
            $( "<li>" ).append( $( "<h2>" ).addClass( "name" ).text( name ) )
         );
      }

      // These limits are global, so that they can be updated by the
      // fix-method on window resize.
   ...
   // Author based highlighting & tag based highlighting
   ( function()
   {
      if ( pageType == BLOG ) {
         return;
      ...
      var linkColorAnimations = { "michael": [], "daniel": [],
                                  "matthias": [], "david": [] };

      var statLabelsAnimation =
         Animation.create( Animation.cssProxy( $( "#stat-labels" ),
                                               "height",
                                               statLabelHeightLimits ) );
      var columnHeightAnimations = { };
      var columnHeightLabels = { };

      // Tag based information: Create labels that display the article amount
      // for the current tag per author.
      $( "#people > li" ).append(
         $( "<div>" ).addClass( "statbar" ).append( $( "<div>" )
                     .addClass( "label" ).append( "0" ) )
      );
      ...
      for ( var i = authors.length; i --> 0; ) {
         ...
         var heightProxy = Animation.cssProxy( qs, "height",
                                               columnHeightLimits );
      ...
      $( "#teaser > li, #people > li" ).each( function( index, item )
      {
         $( item ).mouseover( function( e )
         {
            hightime.trigger( ON, function()
            {
               ...
               var authorAnimations;
               for ( var i = authors.length; i --> 0; ) {
                  if ( authors[ i ] == author ) continue;
                  authorAnimations = linkColorAnimations[ authors[ i ] ];
                  for( var j = authorAnimations.length; j --> 0; )
                     Animation.updateChannels( authorAnimations[ j ],
                                               linkColorDefault );
               }

               authorAnimations = linkColorAnimations[ author ];
               for( i = authorAnimations.length; i --> 0; ) {
                  Animation.updateChannels( authorAnimations[ i ],
                                            linkColors[ author ] )
               }
            } );
         } );
      ...
      $( "#core, #people, #teaser" ).each( function( index, item )
      {
         ...
         {
            if( $( item ).get( 0 ) == e.relatedTarget ) return;
      ...
      $( "#topics > li, #archive > li" ).each( function( index, element )
      {
         ...
         $( element ).mouseout( function( e )
         {
            tooltime.trigger( OFF, function()
            {
               ...
               noStatsTimeout = setTimeout(
                  function() { $( "#people" ).removeClass( "stats" ) }, 550 );
            } );
         } );
   ...
   /**
    * The amount of vertical space which is available to display the gallery
    * browser.
    */
   ...
   function fixGalleryBrowserSize()
   {
      ...
      if ( space.height < browser.height()
           || space.width < browser.width() ) {
   ...
   ( function()
   {
      ...
      function fix()
      {
         ...
         if ( generalContent.height() > 0 ) {
            heights.push( generalContent.height() +
                          generalContent.offset().top + 1 );
         }
   ...
   (function()
   {
      ...
      for ( var i = 0; i < text.length; ++i ) {
         funk += '<span class="c-' + ( (i+1) % FUNKY_NUMBER + 1 ) + '">' +
                 text.charAt( i ) + '</span>';
      }
   ...
   (function()
   {
      ...
      function moveLabel( event )
      {
         ...
         var left = event.clientX - offset.left + 6,
             right = event.clientY - offset.top + 3;
         $( "label", closeButton ).css( "left", left ).css( "top", right );
      }

      var closeButton = $( "<span>" ).attr( "id", "close-gallery" )
                        .html( "&nbsp;" )
